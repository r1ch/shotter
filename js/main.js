const Shotter = {
	data: ()=>({
		...
		colourScale: d3.scaleOrdinal("ABCDEFGHIJKLMNOPQRSTUVWXYZ".split(""),d3.schemeCategory20),
...
ShotterApp.component('map-entry',{
	...
	template: `
		<li class="list-group-item text-white d-flex justify-content-between align-items-center" :style="{background:colourScale(entry.player[0])}">
			{{entry.player}}
	...
	methods: {
		setUp() {
			this.xScale = d3.scaleUtc()
				.domain([0,this.graph[this.graph.length-1].time])
				.range([0, this.width])
